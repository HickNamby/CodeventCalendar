...
if __name__ == '__main__':
    ...
    for key in guardSleepNumbers:
        ...
        if max(guard.getSleepMinutes()) > highScore:
            highScore = max(guard.getSleepMinutes())
            highId = guard.getId()
    ...
    print "ANSWER: " + str(int(highId)*int(guardSleepNumbers[highId].getSleepMinutes().index(max(guardSleepNumbers[highId].getSleepMinutes()))))